    ...
    >
      ...
      <template #file="{ file }">
        ...
        <template v-if="file.type === 'audio'">
          <!-- `http://47.93.191.89:3000${file.url}` -->
          <audio controls :src="file.url" style="margin-top: 42px"></audio>
        </template>
...
import { uploadImg, deletedImg } from "@/api/modules/upload";
...
interface UploadFileProps {
  ...
  delApi?: (params: any) => Promise<any>; // 删除文件的 api 方法，一般项目上传都是同一个 api 方法，在组件里直接引入即可 ==> 非必传
  drag?: boolean; // 是否支持拖拽上传 ==> 非必传（默认为 true）
  disabled?: boolean; // 是否禁用上传组件 ==> 非必传（默认为 false）
  limit?: number; // 最大文件上传数 ==> 非必传（默认为 5个文件）
  fileSize?: number; // 文件大小限制 ==> 非必传（默认为 5M）
  fileType?: File.FileMimeType[]; // 文件类型限制 ==> 非必传（默认为常见的音视频图片各4种格式）
...
const uploadSuccess = (response: { path: string; id: number } | undefined, uploadFile: any) => {
  if (!response) return;
  uploadFile.uid = response.id;
  uploadFile.type = findFileType(response.path);
...
const handlePictureCardPreview: UploadProps["onPreview"] = (file: any) => {
  ...
  if (file.type === "img") {
    ...
    filePreviewRef.value?.openDialog({
      type: file.type,
      url: viewImageUrl
    });
  }
